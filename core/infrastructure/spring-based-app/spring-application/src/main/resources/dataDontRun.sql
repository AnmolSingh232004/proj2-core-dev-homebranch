...
--
--SELECT * FROM DAY_ACTIVITY
--WHERE ID > (SELECT count(*) FROM DAY_ACTIVITY) - 3
--
--SELECT * FROM DAY_ACTIVITY
--LIMIT (SELECT COUNT(*) FROM DAY_ACTIVITY) - 3, 3
--
-- with prepared statement - doesnt work in h2
--SET @rows = 3;
--SET @startRow = (SELECT COUNT(*) FROM DAY_ACTIVITY);
--PREPARE getLastThree FROM 'SELECT * FROM DAY_ACTIVITY LIMIT ?, ?';
--EXECUTE getLastThree USING @startRow , @rows

-- ----- FIND HOW MANY ACTIVITIES ARE OF SAME-----
-- SELECT COUNT(AMOUNT_FACTOR)
-- FROM DAY_ACTIVITY
-- GROUP BY AMOUNT_FACTOR

-- ----- FIND ACTTYPE IDs WHICH HAVE IS_LOGGED VALUES TRUE AND FALSE, ONE AT LEAST-----
-- ------ works without where same ------
--SELECT activitytype_id FROM DAY_ACTIVITY
-- --WHERE is_logged IN ('TRUE', 'FALSE')
--GROUP  BY activitytype_id
--HAVING COUNT(DISTINCT is_logged) = 2
...
-- ----- GROUP BY activity id, the sums of todokg ------
-- SELECT sum(cast(todo_kg as int)), activity_id
-- FROM DAY_ACTIVITY
-- group by activity_id

-- ----- group by Date, the sums of amount-factor, order by date asc------
--SELECT sum(cast(amount_factor as int)), date
--FROM DAY_ACTIVITY
--GROUP BY date
--ORDER BY date DESC

-- ----- GROUP BY date and is_logged ------
-- see is_logged category values(true, false), per date --
--SELECT sum(cast(amount_factor as int)), date, is_logged
--FROM DAY_ACTIVITY
--GROUP BY date, is_logged
--ORDER BY date DESC
-- ]
-- ----- Variation: exclude activity id 5 ------
--SELECT sum(cast(amount_factor as int)), date
--FROM DAY_ACTIVITY
--WHERE activity_id IN (SELECT activity_id FROM day_activity WHERE activity_id <> 5)
--GROUP BY date
--ORDER BY date DESC

-- ----- SIMPLE GROUP BY WITH SUMS OF A/F-----
-- SELECT sum(CAST(amount_factor AS int)), activitytype_id
-- FROM DAY_ACTIVITY
-- group by activitytype_id
...
-- CREATE TABLE visits (`userId` int, `webpageId` int, `visits` int);
--
-- INSERT INTO visits (`userId`, `webpageId`, `visits`)
-- VALUES
--  (0, 123, 1000),
--     (0, 124, 1200),
--   (1, 123, 500),
...
-- QUERIES  1, -------
-- SELECT userId, COUNT(DISTINCT webpageId) AS count, SUM(visits)
-- FROM visits
-- GROUP BY userId
...
-- ---------- JOIN dayActivities with activityType------------
-- ----- Shows more than 4 rows of A/T, as it matches all D/A results for each A/T id --
-- ----- One resulting row is all NULL for D/A, and not null for A/T --
-- ----- If remove "left", becomes inner join, where only rows with not-null columns for BOTH sides appear --
--SELECT * FROM ACTIVITY_TYPE AT
--left JOIN DAY_ACTIVITY DA ON  DA.ACTIVITYTYPE_ID = AT.ID



-- ---------- Show all id/amountFactors which are greater than avg------------
--SELECT amount_factor, id
--FROM DAY_ACTIVITY
--WHERE cast(amount_factor as int) >
--(SELECT avg(cast(amount_factor as int))
-- FROM DAY_ACTIVITY )


-- ---------- Show the rank(first,second..) of each A/F inside each partition(by actType id) ------------
--SELECT
--id, amount_factor, date, activitytype_id ,
--dense_rank() OVER (partition by activitytype_id ORDER BY cast(amount_factor as int) DESC)
-- FROM DAY_ACTIVITY